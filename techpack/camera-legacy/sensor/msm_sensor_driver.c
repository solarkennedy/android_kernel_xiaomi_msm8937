...
extern int legacy_xiaomi_ulysse_match_sensor_eeprom(struct msm_camera_sensor_slave_info *slave_info);
#endif
#if IS_ENABLED(CONFIG_MACH_FAMILY_XIAOMI_WINGTECH)
extern int legacy_xiaomi_wingtech_sensor_match_id(struct msm_camera_sensor_slave_info *slave_info);
extern void legacy_xiaomi_wingtech_sensor_read_fusion_id(struct msm_camera_sensor_slave_info *slave_info, struct msm_sensor_ctrl_t *s_ctrl);
#endif
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if (xiaomi_msm8937_mach_get_family() == XIAOMI_MSM8937_MACH_FAMILY_ULYSSE) {
		rc = legacy_xiaomi_ulysse_match_sensor_eeprom(slave_info);
		if (rc != 0)
			goto free_slave_info;
	}
#endif
	...
	if (xiaomi_msm8937_mach_get_family() == XIAOMI_MSM8937_MACH_FAMILY_WINGTECH) {
		rc = legacy_xiaomi_wingtech_sensor_match_id(slave_info);
		if (rc != 0)
			goto free_slave_info;
	}
#endif
...
#if IS_ENABLED(CONFIG_MACH_FAMILY_XIAOMI_WINGTECH)
	if (xiaomi_msm8937_mach_get_family() == XIAOMI_MSM8937_MACH_FAMILY_WINGTECH)
		legacy_xiaomi_wingtech_sensor_read_fusion_id(slave_info, s_ctrl);
#endif
