...
static void reset(struct bhy_client_data *client_data)
{
	...
	set_fs(old_fs);
	__pm_relax(client_data->reset_wlock);
	return;
...
static void bhy_irq_work_func(struct bhy_client_data *client_data)
{
	...
	if (in_suspend_copy) {
		__pm_stay_awake(client_data->wlock);
		msleep(20);
	}
	...
	const char *buf, size_t count)
{
	...
	/* Register IRQ before reset if not yet done — IRQ signals RESET_DONE. */
	if (!client_data->irq_enabled) {
		ret = bhy_request_irq(client_data);
		if (ret < 0) {
			PERR("Request IRQ failed");
...
static const struct iio_info info = {
	/* .driver_module removed from iio_info in 4.15 */
...
{
	/* iio_sw_buffer_preenable() was removed in 4.14; the IIO core now handles
	 * the timestamp-channel check internally before calling preenable. */
	return 0;
}
...
int init_indio_dev(struct bhy_client_data *client_data)
{
	...
	/* ring->bytes_per_datum direct field access removed in 4.14; the kfifo
	 * buffer derives bytes_per_datum from the scan mask at enable time. */
	iio_device_attach_buffer(client_data->indio, ring);
	client_data->indio->scan_timestamp = true;
...
/*
 * sensors_register/sensors_unregister are Samsung-specific sensors_class
 * helpers not present in this 4.19 tree.  The BST BHy HAL uses the input_dev
 * and bst_dev sysfs paths instead, so these are safe no-ops.
 */
static inline int sensors_register(struct device *dev, void *drvdata,
	struct device_attribute **attrs, const char *name) { return 0; }
static inline void sensors_unregister(struct device *dev,
...
static void bhy_clear_up(struct bhy_client_data *client_data)
{
	if (client_data != NULL) {
		...
		if (client_data->input_attribute_group != NULL) {
			...
			sysfs_remove_group(&client_data->data_bus.dev->kobj,
				client_data->input_attribute_group);
		...
		sysfs_remove_bin_file(&client_data->input->dev.kobj,
			&bin_attr_fifo_frame);
		sysfs_remove_bin_file(&client_data->data_bus.dev->kobj,
			&bin_attr_fifo_frame);
...
#endif /*~ BHY_AR_HAL_SUPPORT */
		wakeup_source_remove(client_data->wlock);
		wakeup_source_destroy(client_data->wlock);
		wakeup_source_remove(client_data->patch_wlock);
		wakeup_source_destroy(client_data->patch_wlock);
		wakeup_source_remove(client_data->reset_wlock);
		wakeup_source_destroy(client_data->reset_wlock);
...
int bhy_probe(struct bhy_data_bus *data_bus)
{
	...
	*/
	...
	/* Also expose attrs on the i2c client device so the BST HAL can find
	 * them at /sys/class/i2c-dev/i2c-2/device/2-0028/ (stock path). */
	(void)sysfs_create_group(&client_data->data_bus.dev->kobj,
		client_data->input_attribute_group);
	/* Expose fifo_frame bin_attr on i2c client too; HAL reads frames here. */
	(void)sysfs_create_bin_file(&client_data->data_bus.dev->kobj,
		&bin_attr_fifo_frame);
	...
	client_data->wlock = wakeup_source_create("bhy");
		wakeup_source_add(client_data->wlock);
	...
	client_data->patch_wlock =
			wakeup_source_create("bhy_patch");
		wakeup_source_add(client_data->patch_wlock);
		client_data->reset_wlock =
			wakeup_source_create("bhy_reset");
		wakeup_source_add(client_data->reset_wlock);
